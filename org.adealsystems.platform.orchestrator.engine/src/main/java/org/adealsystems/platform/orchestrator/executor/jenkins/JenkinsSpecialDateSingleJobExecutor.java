...
public class JenkinsSpecialDateSingleJobExecutor implements SpecialDateSingleJobExecutor {

    private final CommandIdGenerator commandIdGenerator;

    private final JenkinsSpecialDateJobFactory jobFactory;

    public JenkinsSpecialDateSingleJobExecutor(JenkinsSpecialDateJobFactory jobFactory, CommandIdGenerator commandIdGenerator) {
    ...
    public ExecutorResult<ExecutorExitCode> execute(LocalDate inputDate, DataIdentifier dataId) {
        ...
        JenkinsSpecialDateJob job = jobFactory.getJob(inputDate, dataId);
        if (job == null) {
            return new ExecutorResult<>(ExecutorExitCode.UNDEFINED, commandId, "No jenkins job is configured for " + dataId + "!");
        }

        String command = job.createCommand(commandId, inputDate, dataId);
